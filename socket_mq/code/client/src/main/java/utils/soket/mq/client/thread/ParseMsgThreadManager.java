...
import utils.soket.mq.client.ClientMQMsgQueue;
...
public class ParseMsgThreadManager extends AbsThread {
	...
	class ParseThreadManage implements Runnable {
		public void run() {
			while (isStart) {
				try {
					BytesMessage bm = ClientMQMsgQueue.getQueue().take();
					threadPool.execute(new ParseMsgThread(bm));
				} catch (Exception e) {
